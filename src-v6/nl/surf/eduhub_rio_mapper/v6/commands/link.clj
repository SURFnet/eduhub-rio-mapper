...
(ns nl.surf.eduhub-rio-mapper.v6.commands.link
...
(defn make-linker [rio-config getter]
  {:pre [rio-config]}
  (fn [{::rio/keys [opleidingscode aangeboden-opleiding-code] :keys [institution-oin] :as request}]
    ...
    (let [rio-type (if opleidingscode :oe :ao)
          ...
          (fn []
            (getter (rio.loader/rio-entity-request rio-type (or opleidingscode aangeboden-opleiding-code) institution-oin :literal)))]
